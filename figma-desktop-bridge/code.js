...
// Show persistent status panel
figma.showUI(__html__, { width: 300, height: 156, visible: true, themeColors: true });

// Pin plugin to the Design panel right rail (shows under "Plugin" section like Open Loom)
// This persists across sessions — the button stays even after the plugin closes
figma.root.setRelaunchData({ open: '' });

// Relaunch button handler — re-opens UI from Design panel without restarting plugin
figma.on('run', function(event) {
  if (event && event.command === 'open') {
    figma.showUI(__html__, { width: 300, height: 156, visible: true, themeColors: true });
    figma.root.setRelaunchData({ open: '' }); // re-pin on every run to keep it fresh
  }
});
